...
import { ArrowRight, CheckCircle, Leaf, MapPin, Phone, Mail, Hammer, Sprout, Trees, Lightbulb, Images, GraduationCap, Wrench, ShieldCheck, CalendarCheck, ExternalLink, Menu } from "lucide-react";
import type { ReactNode } from "react";

function Button({ children, href, variant = "primary", className = "" }: { children: ReactNode; href: string; variant?: "primary" | "secondary" | "outline"; className?: string }) {
  const base = "inline-flex items-center justify-center rounded-2xl px-6 py-3 text-sm font-bold transition";
  const styles = {
    primary: "bg-green-900 text-white hover:bg-green-950",
    secondary: "bg-white text-green-950 hover:bg-green-50",
    outline: "border border-green-900/25 bg-white/70 text-green-950 hover:bg-white",
  };
...
export default function HoveniersbedrijfJohnKlaassen() {
  ...
  const onderhoud = ["Periodiek begeleidend tuinonderhoud", "Hagen snoeien en vormgeven", "Zomerbeurt, winterbeurt en voorjaarsbeurt", "Gazononderhoud en vijveronderhoud", "Werkzaamheden afgestemd op wat u zelf wilt doen", "Vaste klanten ontvangen jaarlijks een onderhoudsformulier"];

  const projecten = [
    ["Familie Brouwers", "Gilze", "Particuliere tuin", `${base}/albums/14/01_fam_c_brouwers.jpg`],
    ["Plantencentrum Princenbosch", "Molenschot", "Bedrijfsterrein / plantencentrum", `${base}/albums/15/01_plantencentrum_princenbosch.jpg`],
    ["Familie van den Ouweland", "Gilze", "Terras & erf", `${base}/albums/16/01_fam.ouweland.jpg`],
    ["Familie van Eijck", "Ulvenhout", "Moderne tuin", `${base}/albums/17/01_fam_van_eijck.jpg`],
    ["Familie de Vet", "Gilze", "Particuliere tuin", `${base}/albums/18/01_fam_de_vet.jpg`],
    ["Speelboerderij De Vossenberg", "Gilze", "Recreatie & speelomgeving", `${base}/albums/19/01_vossenberg.jpg`],
    ["Familie van Beek", "Gilze", "Particuliere tuin", `${base}/albums/20/01_fam_van_beek.jpg`],
    ["Familie Jac. van Hoek", "Gilze", "Particuliere tuin", `${base}/albums/21/01_fam_jac_van_hoek.jpg`],
    ["Familie de Vet-Roozen", "Gilze", "Particuliere tuin", `${base}/albums/22/01_fam_de_vet_roozen.jpg`],
    ["Edwin de Vet", "Gilze", "Particuliere tuin", `${base}/albums/23/01_edwin_de_vet.jpg`],
  ];

  const kernwaarden = ["Kwaliteit sinds 1994", "Betrouwbaar en persoonlijk contact", "Aanleg, onderhoud, advies en materialen", "Voor particulieren én bedrijven", "Actief in Gilze en omgeving", "Erkend leerbedrijf met aandacht voor vakmanschap"];
    ...
    <div className="min-h-screen bg-[#f6f2e8] text-stone-950">
      <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-[#f6f2e8]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          <a href="#home" className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-green-900 text-white shadow-sm"><Leaf className="h-7 w-7" /></div><div><div className="text-xl font-black leading-none tracking-tight">John Klaassen</div><div className="text-xs font-bold uppercase tracking-[0.2em] text-green-800">Compleet voor uw tuin</div></div></a>
          <nav className="hidden items-center gap-7 text-sm font-bold text-stone-700 xl:flex"><a href="#diensten" className="hover:text-green-800">Diensten</a><a href="#onderhoud" className="hover:text-green-800">Onderhoud</a><a href="#projecten" className="hover:text-green-800">Projecten</a><a href="#over" className="hover:text-green-800">Over ons</a><a href="#vacatures" className="hover:text-green-800">Vacatures</a><a href="#contact" className="hover:text-green-800">Contact</a></nav>
          <div className="flex items-center gap-3"><Button href="tel:0653209296" className="hidden md:inline-flex"><Phone className="mr-2 h-4 w-4" /> Bel direct</Button><div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-green-900/20 bg-white/60 xl:hidden"><Menu className="h-5 w-5" /></div></div>
        </div>
      </header>

      <main id="home">
        <section className="relative overflow-hidden"><div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_15%,rgba(22,101,52,0.22),transparent_32%),radial-gradient(circle_at_10%_80%,rgba(132,204,22,0.16),transparent_28%)]" />
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-28">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
              ...
              <div className="flex flex-col gap-3 sm:flex-row"><Button href="#contact">Bespreek uw tuin <ArrowRight className="ml-2 h-4 w-4" /></Button><Button href="#projecten" variant="outline">Bekijk projecten</Button></div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65, delay: 0.15 }}><div className="relative rounded-[2.5rem] bg-white p-4 shadow-2xl ring-1 ring-stone-200"><img src={`${base}/albums/17/01_fam_van_eijck.jpg`} alt="Moderne tuin aangelegd door Hoveniersbedrijf John Klaassen" className="h-[560px] w-full rounded-[1.9rem] object-cover" /><div className="absolute bottom-8 left-8 right-8 rounded-[1.5rem] bg-white/88 p-5 shadow-xl backdrop-blur"><p className="mb-2 text-sm font-black uppercase tracking-[0.18em] text-green-800">Project uit portfolio</p><h2 className="text-2xl font-black">Moderne tuinaanleg met terras, gazon en beplanting.</h2></div></div></motion.div>
          </div>
        </section>

        <section id="diensten" className="bg-white py-20"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="mb-12 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end"><div><p className="mb-3 font-black uppercase tracking-[0.2em] text-green-800">Aanleg</p><h2 className="text-4xl font-black tracking-tight md:text-5xl">Alles voor een complete tuin, professioneel uitgevoerd.</h2></div><p className="max-w-2xl leading-8 text-stone-600 lg:justify-self-end">Bij aanleg draait het om vakmanschap en materialenkennis. John Klaassen levert materialen, verzorgt de uitvoering en denkt praktisch mee over wat past bij woning, budget en onderhoud.</p></div><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{aanleg.map(({ icon: Icon, title, text }) => <Card key={title} className="group transition duration-300 hover:-translate-y-1 hover:shadow-xl"><div className="p-7"><div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-green-100 text-green-900 transition group-hover:bg-green-900 group-hover:text-white"><Icon className="h-7 w-7" /></div><h3 className="mb-3 text-xl font-black">{title}</h3><p className="leading-7 text-stone-600">{text}</p></div></Card>)}</div></div></section>
        ...
        <section id="projecten" className="bg-stone-950 py-20 text-white"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="mb-12 grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end"><div><p className="mb-3 font-black uppercase tracking-[0.2em] text-lime-300">Projecten</p><h2 className="text-4xl font-black tracking-tight md:text-5xl">Echte projecten uit het bestaande portfolio.</h2></div><p className="max-w-2xl leading-8 text-stone-300 lg:justify-self-end">De oude website bevat tien projectalbums. In deze professionele versie zijn ze zichtbaar als portfolio met foto, plaats, type werk en korte toelichting.</p></div><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{projecten.map(([naam, plaats, type, image], index) => <div key={naam} className="group overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 text-white shadow-none backdrop-blur transition duration-300 hover:-translate-y-1 hover:bg-white/15"><div className="relative h-64 overflow-hidden bg-green-900"><img src={image} alt={`${naam} ${plaats}`} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute left-4 top-4 rounded-full bg-stone-950/70 px-3 py-1 text-xs font-black uppercase tracking-[0.14em] text-lime-200 backdrop-blur">Project {String(index + 1).padStart(2, "0")}</div></div><div className="p-6"><div className="mb-3 flex items-center gap-2 text-sm font-bold text-lime-200"><Images className="h-4 w-4" /> {type}</div><h3 className="mb-1 text-2xl font-black">{naam}</h3><p className="mb-4 flex items-center gap-2 text-sm font-semibold text-stone-300"><MapPin className="h-4 w-4" /> {plaats}</p><p className="leading-7 text-stone-300">Project uit het portfolio van Hoveniersbedrijf John Klaassen met aandacht voor groen, bestrating en dagelijks gebruiksgemak.</p></div></div>)}</div></div></section>

        <section id="over" className="bg-white py-20"><div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[0.9fr_1.1fr] lg:px-8"><div className="rounded-[2.25rem] bg-green-950 p-8 text-white shadow-xl md:p-10"><p className="mb-4 font-black uppercase tracking-[0.2em] text-lime-300">Over ons</p><h2 className="mb-6 text-4xl font-black tracking-tight">Alles rondom de tuin uit handen nemen.</h2><p className="mb-8 leading-8 text-green-50">Sinds 1994 staat Hoveniersbedrijf John Klaassen voor kwaliteit, betrouwbaarheid en persoonlijk contact. Het bedrijf ontwerpt, verwijdert, plant, bestraat, plaatst schuttingen en levert benodigde materialen.</p><div className="grid gap-3 sm:grid-cols-2">{kernwaarden.map(waarde => <div key={waarde} className="flex gap-3 rounded-2xl bg-white/10 p-4"><CheckCircle className="mt-0.5 h-5 w-5 shrink-0 text-lime-300" /><p className="font-semibold leading-6 text-green-50">{waarde}</p></div>)}</div></div><div className="grid gap-6 md:grid-cols-2"><Card><div className="p-7"><GraduationCap className="mb-5 h-10 w-10 text-green-900" /><h3 className="mb-3 text-2xl font-black">Erkend leerbedrijf</h3><p className="leading-7 text-stone-600">Jongeren doen ervaring op met alle voorkomende werkzaamheden, met aandacht voor werkhouding, gereedschap en machines.</p></div></Card><Card><div className="p-7"><Wrench className="mb-5 h-10 w-10 text-green-900" /><h3 className="mb-3 text-2xl font-black">Eigen machines</h3><p className="leading-7 text-stone-600">Het bedrijf werkt praktisch en vakgericht, met kennis van machines, materialen en onderhoud.</p></div></Card><div className="overflow-hidden rounded-[2rem] shadow-xl md:col-span-2"><img src={`${base}/albums/14/01_fam_c_brouwers.jpg`} alt="Tuinaanleg Familie Brouwers Gilze" className="h-[320px] w-full object-cover" /></div></div></div></section>
        ...
        <section id="contact" className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="overflow-hidden rounded-[2.5rem] bg-green-950 text-white shadow-2xl"><div className="grid lg:grid-cols-[1.05fr_0.95fr]"><div className="p-8 md:p-12"><p className="mb-3 font-black uppercase tracking-[0.2em] text-lime-300">Contact</p><h2 className="mb-5 text-4xl font-black tracking-tight md:text-5xl">Nieuwsgierig naar de mogelijkheden voor uw tuin?</h2><p className="mb-8 max-w-xl leading-8 text-green-50">Neem contact op voor aanleg, onderhoud, advies of vacatures. Overdag is het bedrijf het beste telefonisch bereikbaar.</p><div className="flex flex-col gap-3 sm:flex-row"><Button href="tel:0653209296" variant="secondary"><Phone className="mr-2 h-4 w-4" /> 06 53 20 92 96</Button><a href="mailto:john@hoveniersbedrijfjohnklaassen.nl" className="inline-flex items-center justify-center rounded-2xl border border-white/30 px-6 py-3 text-sm font-bold text-white hover:bg-white/10"><Mail className="mr-2 h-4 w-4" /> Mail ons</a></div></div><div className="bg-white/10 p-8 md:p-12"><div className="rounded-[2rem] bg-white p-7 text-stone-950 shadow-xl"><h3 className="mb-6 text-2xl font-black">Hoveniersbedrijf John Klaassen</h3><div className="space-y-5 text-stone-700"><p className="flex gap-3"><MapPin className="mt-1 h-5 w-5 shrink-0 text-green-800" /> Vossenberg 11<br />5126 PE Gilze</p><p className="flex items-center gap-3"><Phone className="h-5 w-5 text-green-800" /> 06 53 20 92 96</p><p className="flex items-center gap-3"><Mail className="h-5 w-5 text-green-800" /> john@hoveniersbedrijfjohnklaassen.nl</p><p className="text-sm text-stone-500">Fax: 013 508 12 36</p></div></div></div></div></div></section>
      </main>
    </div>
  );
}
